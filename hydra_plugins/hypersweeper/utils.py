...
def convert_to_configuration(x: pd.Series, configspace: ConfigurationSpace) -> Configuration:
    ...
    x = dict(x)
    hp_config = {
        k: maybe_convert_types(k, v, configspace=configspace) for k, v in x.items() if k.startswith("hp_config") and not np.isnan(v)
    }
    return Configuration(configuration_space=configspace, values=hp_config)


def read_warmstart_data(warmstart_filename: str, search_space: ConfigurationSpace, maximize: bool) -> list[tuple[Info, Result]]:
